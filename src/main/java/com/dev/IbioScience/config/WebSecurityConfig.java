...
public class WebSecurityConfig {
	...
	private static final String[] STATIC_CUSTOM_IGNORES = { "/front/**", "/administration/**" };

	private static final String[] AUTH_WHITELIST = { "/", "/index", "/signIn", "/signup",
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http.csrf(csrf -> csrf.disable()).headers(h -> h.frameOptions(f -> f.disable()))
				...
				.authorizeHttpRequests(auth -> auth.requestMatchers(ADMIN_ROOT_URLS).hasRole("ROOT")
						...
						.requestMatchers(ADMIN_COMMON_URLS).hasAnyRole("ADMIN", "OPERATOR", "MASTER", "ROOT")
				...
				.formLogin(form -> form.loginPage("/signIn").loginProcessingUrl("/signInProcess")
						// defaultSuccessUrl 대신 커스텀 성공/실패 핸들러 사용
						.successHandler(customAuthSuccessHandler).failureHandler(customAuthFailureHandler).permitAll())
