...
(function() {
	...
	const st = {
		...
		shippingMethod: "택배배송",
		shipPay: "선불",
		expectPointRate: 0.10, // ✅ 서버(OrderService)와 동일하게 10%
		shipFee: 0,
		itemDiscountTotal: 0
	};
	...
	function buildCreateOrderRequest() {
		...
		$$(".paymentSuccess-orderTable tbody tr").forEach((tr, idx) => {
			...
			const itemProductType = normalizeItemProductType(origin.itemProductType || origin.productType);
	...
	// 계산 핵심
	// =========================
	function getRowsWithOrigin() {
		const rows = $$(".paymentSuccess-orderTable tbody tr");
		return rows.map((tr, idx) => ({
			tr,
			origin: st.payload && st.payload.items ? st.payload.items[idx] : null
		}));
	}

	function calcTotalsByProductType() {
		let companySum = 0;
		let dealerSum = 0;
		let totalCount = 0;

		getRowsWithOrigin().forEach(({ tr, origin }) => {
			const unitPrice = num(tr.querySelector(".ps-unitPrice")?.dataset?.value);
			const qty = Math.max(1, num(tr.querySelector(".paymentSuccess-qty-input")?.value));
			const line = unitPrice * qty;
			const type = normalizeItemProductType(origin?.itemProductType || origin?.productType);

			if (type === 'DEALER') dealerSum += line;
			else companySum += line;
		...
		return {
			companySum,
			dealerSum,
			totalSum: companySum + dealerSum,
			totalCount
		};
	}

	function couponDiscount(companySumPrice) {
		if (!st.coupon) return 0;
		if (companySumPrice <= 0) return 0;

		const min = num(st.coupon.minPurchaseAmount);
		if (companySumPrice < min) return 0;

		const amount = num(st.coupon.couponAmount);
		return Math.min(amount, companySumPrice);
	}

	function syncPointInputValue() {
		const input = $("#paymentSuccess-pointUse");
		if (!input) return;
		input.value = st.pointUse ? fmt(st.pointUse) : "";
	}

	function recalc() {
		updateShipFeeFromMethod();

		const totals = calcTotalsByProductType();
		const sumPrice = totals.totalSum;
		const companySumPrice = totals.companySum;
		const dealerSumPrice = totals.dealerSum;

		const baseDiscount = 0;
		const couponDc = couponDiscount(companySumPrice);

		const maxPointApplicable = Math.max(0, companySumPrice - baseDiscount - couponDc);
		let pointDc = Math.min(num(st.pointUse), num(st.pointAvail));
		if (pointDc > maxPointApplicable) {
			pointDc = maxPointApplicable;
			st.pointUse = pointDc;
			syncPointInputValue();
		}
		...
		const companyPayable = Math.max(0, companySumPrice - baseDiscount - couponDc - pointDc);
		const dealerPayable = Math.max(0, dealerSumPrice);
		const grand = companyPayable + dealerPayable + shipToPayNow;
		...
		if (sumShipEl) sumShipEl.textContent = fmt(shipFee);
		...
		if (totalCountEl) totalCountEl.textContent = String(totals.totalCount);
		...
		// ✅ 회사상품 금액 기준으로만 예상 적립 포인트 계산
		const expectPointEl = document.querySelector('[data-bind="expectPoint"]');
		if (expectPointEl) expectPointEl.textContent = fmt(Math.floor(companySumPrice * st.expectPointRate));

		const list = $("#paymentSuccess-discountList");
		if (list) {
			const shipText = includeShip ? `+ ${fmt(shipFee)} 원` : `+ ${fmt(shipFee)} 원 (착불)`;
			list.innerHTML = `
				<li class="d-flex justify-content-between"><span>회사상품 합계</span><span>${fmt(companySumPrice)} 원</span></li>
				<li class="d-flex justify-content-between"><span>딜러상품 합계</span><span>${fmt(dealerSumPrice)} 원</span></li>
				<li class="d-flex justify-content-between"><span>기본 프로모션</span><span>- ${fmt(baseDiscount)} 원</span></li>
				<li class="d-flex justify-content-between"><span>쿠폰 할인(회사상품만)</span><span>- ${fmt(couponDc)} 원</span></li>
				<li class="d-flex justify-content-between"><span>적립금 사용(회사상품만)</span><span>- ${fmt(pointDc)} 원</span></li>
	...
	function renderCouponList(list, modal) {
		...
		ul.querySelectorAll(".paymentSuccess-cpnCheck").forEach(chk => {
			chk.addEventListener("change", (e) => {
				if (e.target.checked) {
					ul.querySelectorAll(".paymentSuccess-cpnCheck").forEach(other => {
	...
	function attachCouponModal() {
		...
		if (applyBtn) {
			applyBtn.addEventListener("click", () => {
				...
				const totals = calcTotalsByProductType();
				const companySumPrice = totals.companySum;
				const min = num(c.minPurchaseAmount);

				if (companySumPrice <= 0) {
					alert("쿠폰은 우리회사 상품이 포함된 주문에만 적용할 수 있습니다.");
					return;
				}

				if (companySumPrice < min) {
					alert(`선택한 쿠폰은 우리회사 상품 구매금액이 ${fmt(min)}원 이상일 때만 사용할 수 있습니다.`);
					return;
				}
