...
// === 공통표시항목 알림 제어 ===
const noticeEl = document.getElementById('question-global-notice');
let noticeClosed = sessionStorage.getItem('displayNoticeClosed') === '1';

function showNoticeIfNeeded() {
	if (!noticeClosed && noticeEl) {
		noticeEl.classList.remove('d-none');
		// fade-in
		setTimeout(() => noticeEl.classList.add('show'), 10);
	}
}

// 부트스트랩 alert 닫힘 감지 -> 세션 동안 다시 안 뜨게
document.addEventListener('closed.bs.alert', function(e) {
	if (e.target === noticeEl) {
		noticeClosed = true;
		sessionStorage.setItem('displayNoticeClosed', '1');
	}
});

// setDirty / setClean 훅에 연결
const _setDirtyOrig = setDirty;
const _setCleanOrig = setClean;
