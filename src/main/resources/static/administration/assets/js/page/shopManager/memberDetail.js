...
/* ===========================================================
 * 5) (선택) 상세화면 JS — 아이디 중복체크(셀프일 때만 버튼 활성)
 * - username 인풋이 readOnly가 아니면(=본인) 중복체크 버튼 동작
 * - ROOT/MASTER가 타인 정보를 수정할 때는 username이 readOnly → 중복체크 요구/동작하지 않음
 * - 상세 화면에서는 '중복체크를 필수로 강제'하지 않습니다. (서버에서 최종 검증)
 * =========================================================== */
(() => {
	const form = document.querySelector("#member-detail-form");
	if (!form) return;
	...
	const idCheckBtn = document.querySelector("#member-detail-idCheckBtn");
	const idCheckResult = document.querySelector("#member-detail-idCheckResult");

	// username이 읽기전용(=본인 아님)이면 중복체크 이벤트를 붙이지 않음
	if (!usernameInput || usernameInput.readOnly || !idCheckBtn || idCheckBtn.disabled) {
		return;
	}
